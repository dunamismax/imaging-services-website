...
<section class="mx-auto mt-10 max-w-7xl px-4 lg:px-8">
    ...
    <div class="mt-5 grid gap-3">
        @foreach ($equipment['intro'] as $line)
            <p class="text-base text-brand-muted" wire:key="equipment-intro-{{ $loop->index }}">{{ $line }}</p>
        @endforeach
    </div>
</section>
...
<section class="mx-auto mt-10 max-w-7xl px-4 lg:px-8">
    <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        @foreach ($equipment['categories'] as $item)
            <article class="surface-card overflow-hidden" wire:key="equipment-category-{{ $item['slug'] }}">
    ...
    <div class="surface-card p-6 md:p-8">
        ...
        <div class="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            @foreach ($partners as $partner)
                <div class="flex items-center justify-center rounded-xl border border-brand-ink/10 bg-white p-3" wire:key="equipment-partner-{{ $partner['name'] }}">
