        ...
        <article class="surface-card p-6 md:p-8">
            ...
            <ul class="mt-4 space-y-2">
                @foreach ($services['service_options'] as $option)
                    <li class="rounded-xl border border-brand-ink/10 bg-white px-4 py-3 text-sm text-brand-muted">{{ $option }}</li>
                @endforeach
            </ul>
        </article>
        ...
        <article class="surface-card p-6 md:p-8">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                @foreach ($services['brands'] as $brand)
                    <span class="rounded-full bg-brand-soft px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-accent">{{ $brand }}</span>
                @endforeach
            </div>
        </article>
    </div>
...
<section class="mx-auto mt-10 max-w-7xl px-4 lg:px-8">
    <div class="grid gap-6 xl:grid-cols-2">
        @include('forms.service-request', ['options' => $services['service_options']])
        @include('forms.training-request', ['options' => $services['training_options']])
    </div>
</section>
