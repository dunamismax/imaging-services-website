        ...
        <article class="surface-card p-6 lg:col-span-2">
            ...
            <div class="mt-3 flex flex-wrap gap-3">
                @foreach ($contact['principals'] as $person)
                    <a href="tel:{{ preg_replace('/[^\d+]/', '', $person['phone']) }}" class="btn-secondary">{{ $person['name'] }}: {{ $person['phone'] }}</a>
                @endforeach
            </div>
...
<section class="mx-auto mt-10 max-w-7xl px-4 lg:px-8">
    <div class="grid gap-6 xl:grid-cols-2">
        @include('forms.contact-sales')
        @include('forms.service-request', ['options' => $serviceOptions])
    </div>
</section>

<section class="mx-auto mt-6 max-w-7xl px-4 lg:px-8">
    @include('forms.training-request', ['options' => $trainingOptions])
</section>
