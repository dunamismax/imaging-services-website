        ...
        <article class="surface-card p-6 md:p-8">
            ...
            <ul class="mt-4 space-y-3">
                @foreach ($accessories['notes'] as $note)
                    <li class="rounded-xl border border-brand-ink/10 bg-white px-4 py-3 text-sm text-brand-muted" wire:key="accessories-note-{{ $loop->index }}">{{ $note }}</li>
                @endforeach
            </ul>
    ...
    <div class="surface-card p-6 md:p-8">
        ...
        <div class="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            @foreach ($partners as $partner)
                <div class="flex items-center justify-center rounded-xl border border-brand-ink/10 bg-white p-3" wire:key="accessories-partner-{{ $partner['name'] }}">
