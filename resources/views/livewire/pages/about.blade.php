...
<section class="mx-auto mt-10 max-w-7xl px-4 lg:px-8">
    ...
    <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        @foreach ($about['team'] as $member)
            <article class="surface-card overflow-hidden" wire:key="team-member-{{ $member['name'] }}">
    ...
    <div class="surface-card p-6 md:p-8">
        ...
        <div class="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            @foreach ($partners as $partner)
                <div class="flex items-center justify-center rounded-xl border border-brand-ink/10 bg-white p-3" wire:key="about-partner-{{ $partner['name'] }}">
